...
| **bigint IDENTITY** (1, 2, 3…) | `questions`, `question_options`, `change_log`, `claude_messages`, `ticket_replies` |

El trigger `BEFORE INSERT` `trg_six_digit_id` asigna el número en `users` y `vacation_requests` si el `INSERT` no trae `id`. `work_areas` y `support_tickets` usan `trg_four_digit_id`. El catálogo y los hilos usan `GENERATED BY DEFAULT AS IDENTITY`. No hay generador de IDs de usuario en Node: el registro y RRHH hacen `INSERT … RETURNING id`.
