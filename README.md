...
| Módulo | Chile | Perú |
|--------|:-----:|:----:|
| Noticias, Academy, RRHH, vacaciones, tickets, procesos, apps, perfil | sí | sí |
...
Los IDs numéricos (usuarios, noticias, tickets, cursos, etc.) son **6 dígitos aleatorios** (100000–999999), no 1, 2, 3. Un trigger `BEFORE INSERT` los asigna si el `INSERT` no trae id; `src/utils/userId.js` hace lo mismo para usuarios nuevos. Las filas que ya existían con serial se remapearon. Las URLs por id numérico antiguas dejan de coincidir; las noticias también se pueden abrir por slug.
...
Cierre de tickets (1 h), limpieza de `change_log` (12 h), transiciones de vacaciones (12 h), caché de home —divisas, clima, LinkedIn en Chile— (15 min).
